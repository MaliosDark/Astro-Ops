...
try {
    switch ($action) {
        ...
        case 'earnings':
            ...
            // Check cache first for earnings data
            if ($cache) {
                $cacheKey = "earnings_" . $me['userId'];
                $cached = $cache->get($cacheKey, 300); // 5 minutes cache
            ...
            try {
                ...
                // Get recent transactions (combined query for better performance)
                $stmt = $pdo->prepare("
                    ...
                    LIMIT 20
                ");
                $stmt->execute([$me['userId'], $me['userId'], $me['userId']]);
                ...
                handleApiError($e->getMessage(), 'earnings');
                jsonErr('Failed to retrieve earnings data', 500);
            }
            break;
        ...
        case 'withdraw_tokens':
            ...
            try {
                ...
                if ($cache) {
                    $cache->delete("earnings_" . $me['userId']);
                }
                ...
                $pdo->rollback();
                handleApiError($e->getMessage(), 'withdraw_tokens');
                jsonErr('Withdrawal failed', 500);
            }
            break;
